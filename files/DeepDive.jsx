...
export default function DeepDive() {
  ...
  const uniBarData = unis.map(u => ({
    name:       u.university.replace('University of ', 'U. of ').replace('Australian National University', 'ANU').replace('UNSW Sydney', 'UNSW'),
    full:       u.university,
  ...
  return (
    <div>
      ...
      <Tabs defaultValue="universities">
        <TabsList className="mb-8 bg-transparent border-b border-rule w-full justify-start rounded-none h-auto p-0 gap-0">
          {['universities', 'fields'].map(tab => (
            <TabsTrigger key={tab} value={tab}
              className="font-mono text-[0.68rem] tracking-widest uppercase px-5 py-3 rounded-none border-b-2 border-transparent data-[state=active]:border-female data-[state=active]:text-ink data-[state=active]:shadow-none data-[state=active]:bg-transparent text-ink3 transition-colors"
            >
          ...
          <ChartGrid cols={1}>
            ...
            >
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={uniBarData} margin={{ right: 40 }}>
          ...
          <ChartGrid cols={2}>
            ...
            >
              <ResponsiveContainer width="100%" height={380}>
                <BarChart data={fieldData} layout="vertical" margin={{ left: 8, right: 48 }}>
                  <CartesianGrid {...GRID} horizontal={false} />
                  <XAxis type="number" tick={TICK} tickFormatter={v => v + '%'} axisLine={false} tickLine={false} />
                  <YAxis type="category" dataKey="field" tick={{ ...TICK, fill: '#5C5650' }} axisLine={false} tickLine={false} width={160} />
                  <Tooltip content={<ChartTooltip formatter={v => v?.toFixed(1) + '%'} />} />
